...
func New(cfg config.Config, db *gorm.DB, redisClient *redis.Client, logger *slog.Logger) *gin.Engine {
	...
	dispatchPermitService := service.NewDispatchPermitService(dispatchPermitRepository, operationDirectiveRepository, gateUnitRepository, reservoirRepository, securityService)
	...
	systemHandler := handler.NewSystemHandler(securityService, reservoirService, gateUnitService, operationDirectiveService, executionConfirmationService, dispatchPermitService, db, redisClient)
