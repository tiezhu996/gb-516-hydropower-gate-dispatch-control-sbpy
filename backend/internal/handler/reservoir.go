...
type ReservoirHandler struct{ service service.ReservoirService }

func NewReservoirHandler(s service.ReservoirService) *ReservoirHandler {
	return &ReservoirHandler{service: s}
}

func (h *ReservoirHandler) Register(group *gin.RouterGroup) {
	resource := group.Group("/reservoirs")
	resource.GET("", h.list)
...
func (h *ReservoirHandler) list(c *gin.Context) {
	query := bindPage(c)
...
func (h *ReservoirHandler) get(c *gin.Context) {
	id, ok := parseID(c)
	if !ok {
		return
	}
...
func (h *ReservoirHandler) create(c *gin.Context) {
	var input dto.CreateReservoir
...
func (h *ReservoirHandler) update(c *gin.Context) {
	id, ok := parseID(c)
	if !ok {
		return
	}
	var input dto.UpdateReservoir
	if err := c.ShouldBindJSON(&input); err != nil {
		util.Fail(c, http.StatusBadRequest, "invalid_request", err.Error())
		return
	}
...
func (h *ReservoirHandler) transition(c *gin.Context) {
	id, ok := parseID(c)
	if !ok {
		return
	}
...
func (h *ReservoirHandler) remove(c *gin.Context) {
