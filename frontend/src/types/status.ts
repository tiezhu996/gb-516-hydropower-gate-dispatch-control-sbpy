...
export type PermitState = 'pending' | 'approved' | 'active' | 'rejected' | 'invalidated' | 'expired';
export const ALL_PERMIT_STATE: readonly PermitState[] = ['pending', 'approved', 'active', 'rejected', 'invalidated', 'expired'];
...
export const ENTITY_TRANSITIONS: Readonly<Record<string, Readonly<Record<string, readonly string[]>>>> = {
	...
	dispatchPermit: {
		...
		approved: ['active', 'rejected', 'expired', 'invalidated'],
...
export const ENTITY_CONFIGS: readonly EntityConfig[] = [
  ...
  { key: 'dispatchPermit', path: 'permits', label: '调度许可', statuses: ALL_PERMIT_STATE },
];
